...
class RegistrationFlags:
    ...
    def can_activate(self):
        return CarerRegistration.objects.has_deactivated_carer(
            self.primary_carer, self.patient
        )
    ...
    def has_expired_registration(self):
        return CarerRegistration.objects.has_expired_registration(
            self.primary_carer, self.patient
        )

    @cached_property
    def has_pending_registration(self):
        return CarerRegistration.objects.has_pending_registration(
            self.primary_carer, self.patient
        )

    @cached_property
    def is_carer_registered(self):
        return CarerRegistration.objects.is_carer_registered(
            self.primary_carer, self.patient
        )
    ...
    def can_register(self):
        return (
            not (
                ...
                or self.can_deactivate
                or self.can_activate
            )
