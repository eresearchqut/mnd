...
class PatientInsuranceForm(PrefixedModelForm):
    has_private_health_fund = forms.BooleanField(
    ...
    ndis_plan_manager = forms.ChoiceField(
        choices=PatientInsurance.PLAN_MANAGER_CHOICES,
        required=False,
        widget=widgets.RadioSelect,
        label=_("NDIS Plan Manager"),
    )
    dva_card_type = forms.ChoiceField(
        choices=PatientInsurance.DVA_CARD_TYPE_CHOICES,
        required=False,
        widget=widgets.RadioSelect,
    ...
    referred_for_mac_care = forms.BooleanField(
        ...
        required=False,
        label=_(
            "Have you been referred for aged care support services via My Aged Care (MAC)?"
        ),
    )
    needed_mac_level = forms.ChoiceField(
        ...
        widget=widgets.RadioSelect,
        label=_(
            "What level of Home Care Package were you assessed as needing?"
        ),
    )
    eligible_for_home_care = forms.BooleanField(
        ...
        required=False,
        label=_(
            "Have you been assessed as being eligible for a community home care package?"
        ),
    )
    ...
    class Meta:
        ...
        fields = (
            ...
            "main_hospital_mrn",
            "secondary_hospital",
            "secondary_hospital_mrn",
            "medicare_number",
            "pension_number",
            "has_private_health_fund",
            "private_health_fund",
            ...
            "ndis_number",
            "ndis_plan_manager",
            "ndis_coordinator_first_name",
            "ndis_coordinator_last_name",
            "ndis_coordinator_phone",
            "ndis_coordinator_email",
            "has_dva_card",
            "dva_card_number",
            "dva_card_type",
            "referred_for_mac_care",
            "eligible_for_home_care",
            "needed_mac_level",
            "receiving_home_care",
            "home_care_level",
        )
        labels = {
            ...
            "ndis_coordinator_first_name": _(
                "NDIS support coordinator first name"
            ),
            "ndis_coordinator_last_name": _(
                "NDIS support coordinator last name"
            ),
    ...
    def clean_ndis_number(self):
        ...
        if ndis_number and (not digits_only or len(ndis_number) != 9):
            raise forms.ValidationError(
                _("NDIS number should contain 9 digits")
            )
        return ndis_number
...
class PrimaryCarerForm(PrefixedModelForm):
    relationship = forms.ChoiceField(
        choices=PrimaryCarerRelationship.PRIMARY_CARER_RELATIONS,
        required=False,
    )
    ...
    class Meta:
        ...
        fields = (
            ...
            "em_contact_first_name",
            "em_contact_last_name",
            "em_contact_phone",
        )
        labels = {
            "is_emergency_contact": _(
                "Is the primary carer the emergency contact number"
            ),
    ...
    def _clean_fields(self):
        required_relationship_info = (
            self.data.get(self.field_name("relationship"), "") == "other"
        )
        self.fields["relationship_info"].required = required_relationship_info
        interpreter_required = (
            self.data.get(self.field_name("interpreter_required"), "") == "True"
        )
    ...
    def set_patient(self, patient):
        ...
        if self.has_assigned_carer():
            ...
            notification = _("""You can't change the personal details of the principal caregiver while it is linked.
                     To unlink the principal caregiver please use the Carer Management menu!""")
            self.fields["first_name"].help_text = mark_safe(
                f'<span style="color:green;"><strong>{notification} </strong></span>'
            )
    ...
    def clean_em_contact_phone(self):
        ...
        if not self._is_valid_phone(em_contact_phone):
            raise forms.ValidationError(
                _("The emergency contact phone should contain digits only")
            )
    ...
    def clean_home_phone(self):
        ...
        if not self._is_valid_phone(phone):
            raise forms.ValidationError(
                _("The phone number should contain digits only")
            )
        return phone
    ...
    def clean_email(self):
        ...
        existing_user = CustomUser.objects.filter(
            username__iexact=email.lower(), is_active=True
        ).first()
        if existing_user and not existing_user.is_carer:
            raise forms.ValidationError(
                _("The email address is already registered into the system")
            )

        if (self.patient_email and self.patient_email == email) or (
            self.patient and self.patient.email == email
        ):
            raise forms.ValidationError(
                _(
                    "Principal carer email and patient contact email should not be the same"
                )
            )
    ...
    def save(self, commit=True):
        ...
        if "email" in self.changed_data:
            existing_carer = PrimaryCarer.objects.filter(
                email__iexact=email
            ).first()
            if existing_carer:
        ...
        if self.patient and commit:
            ...
            pc, __ = PrimaryCarerRelationship.objects.get_or_create(
...
class PreferredContactForm(PrefixedModelForm):
    contact_method = forms.ChoiceField(
        choices=PreferredContact.CONTACT_METHOD_CHOICES,
        required=True,
        widget=widgets.RadioSelect,
    )
    ...
    def _clean_fields(self):
        required_info = (
            self.data.get(self.field_name("contact_method"), "") == "person"
        )
        if required_info:
...
class DuplicatePatientForm(PrefixedModelForm):
    is_duplicate = forms.BooleanField(
        label=_("Mark this patient as a potential duplicate"), required=False
    )
