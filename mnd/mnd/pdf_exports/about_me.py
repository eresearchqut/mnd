...
def _gender_mapping(gender):
    mappings = {"1": "Male", "2": "Female", "3": "Other"}
...
def _generate_patient_fields(patient):
    return {
        ...
        "pDOB": patient.date_of_birth.strftime("%d/%m/%Y")
...
def _generate_patient_address_fields(patient_address):
    ...
    return {
        ...
        "pState": state_value_mapping.get(patient_address.state, "QLD")
...
def _generate_primary_carer_fields(primary_carer, patient, patient_address):
    def primary_carer_relationship(primary_carer):
        ...
        relation = PrimaryCarerRelationship.objects.filter(
    ...
    def primary_carer_address(primary_carer, patient_address):
        ...
        address = (
            patient_adr if primary_carer.same_address else primary_carer.address
        )
        suburb = (
            patient_suburb
            if primary_carer.same_address
            else primary_carer.suburb
        )
        postcode = (
            patient_postcode
            if primary_carer.same_address
            else primary_carer.postcode
        )
        return {
            "p2Address": address,
            "p2Suburb": suburb,
