...
class FormValueResolver:
    def __init__(self, cde_codes):
        ...
        self.cde_values_mapping = {
            cde.code: cde.pv_group.cde_values_dict
            for cde in self.cdes.filter(pv_group__isnull=False)
        }
        self.mims_products = {
            cde.code
            for cde in self.cdes.filter(widget_name="ProductLookupWidget")
        }
        self.mims_cmis = {
            cde.code for cde in self.cdes.filter(widget_name="CMILookupWidget")
        }
