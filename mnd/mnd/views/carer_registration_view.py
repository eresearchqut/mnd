...
from mnd.primary_carer.carer_state_machine import (
    get_carer_state,
    get_current_state,
)
...
class CarerOperationsView(View):
    def get(self, request):
        ...
        primary_carer = PrimaryCarer.objects.filter(
            email__iexact=request.user.email
        ).first()
        return render(
            request,
            "registration/carer_self_operations.html",
            context={"carer": primary_carer, "patients": patients},
        )

    def post(self, request):
        if not request.user.is_carer:
            return HttpResponseForbidden()
        primary_carer = PrimaryCarer.objects.get(
            email__iexact=request.user.email
        )
