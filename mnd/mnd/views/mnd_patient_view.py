...
from rdrf.views.patient_view import AddPatientView as ParentAddPatientView
from rdrf.views.patient_view import PatientEditView as ParentEditPatientView
from rdrf.views.patient_view import PatientFormMixin
...
def get_section(
    ...
    if request.POST:
        ...
        form_instance = form(
            initial=initial or {}, prefix=section_prefix, instance=instance
        )
...
def get_form(form, request, prefix, instance=None, initial=None, patient=None):
    form_instance = form(
        request.POST, prefix=prefix, instance=instance, initial=initial or {}
    )
    if patient:
        form_instance.set_patient(patient)
...
class FormSectionMixin(PatientFormMixin):
    ...
    def get_form_sections(
        ...
        form_sections.insert(
            ...
            get_section(
                ...
                _("Language Info"),
                "language_info",
                get_patient_language(patient),
                request,
            ),
        )
        form_sections.insert(
            3, (patient_form, (mnd_builder.get_contact_details_fields(),))
        )
        ...
        form_sections.insert(
            ...
            get_section(
                PreferredContactForm,
                _("Patient Preferred Contact Method "),
                "preferred_contact",
                get_preferred_contact(patient),
                request,
            ),
        )
        ...
        form_sections.extend(
            [
                get_section(
                    PatientInsuranceForm,
                    _("Medicare, Health Insurance and Support details"),
                    "patient_insurance",
                    get_insurance_data(patient),
                    request,
                ),
                get_section(
                    PrimaryCarerForm,
    ...
    def get_forms(self, request, registry_model, user, instance=None):
        ...
        forms[self.PATIENT_LANGUAGE_KEY] = get_form(
            PatientLanguageForm,
            request,
            "language_info",
            get_patient_language(instance),
        )
        forms[self.PATIENT_INSURANCE_KEY] = get_form(
            PatientInsuranceForm,
            request,
            "patient_insurance",
            get_insurance_data(instance),
        )
        forms[self.PRIMARY_CARER_KEY] = get_form(
            PrimaryCarerForm,
            request,
            "primary_carer",
            get_primary_carer(instance),
            get_primary_carer_initial_data(instance),
            instance,
        )
        forms[self.PREFERRED_CONTACT_KEY] = get_form(
            PreferredContactForm,
            request,
            "preferred_contact",
            get_preferred_contact(instance),
        )
        forms[self.DUPLICATE_PATIENT_KEY] = get_form(
            DuplicatePatientForm,
            request,
            "duplicate_patient",
            get_duplicate_patient(instance),
        )
        return forms
    ...
    def _handle_primary_carer_relationship(self, form, instance):
        ...
        carer = (
            instance or PrimaryCarer.objects.filter(email__iexact=email).first()
        )
        if carer:
            pc = PrimaryCarerRelationship.objects.filter(
                patient=self.object
            ).first()
            if pc is None:
                pc = PrimaryCarerRelationship.objects.create(
                    patient=self.object, carer=carer
                )
            else:
                pc.carer = carer
            pc.relationship = rel
    ...
    def _handle_duplicate_patients(self, form, instance):
        if (
            "is_duplicate" in form.changed_data
            and form.cleaned_data["is_duplicate"]
        ):
            registry_code = instance.patient.rdrf_registry.first().code
            process_notification(
                registry_code,
                EventType.DUPLICATE_PATIENT_SET,
    ...
    def all_forms_valid(self, forms):
        ...
        formset_keys = [
            self.PATIENT_LANGUAGE_KEY,
            self.PATIENT_INSURANCE_KEY,
            self.PRIMARY_CARER_KEY,
            self.PREFERRED_CONTACT_KEY,
